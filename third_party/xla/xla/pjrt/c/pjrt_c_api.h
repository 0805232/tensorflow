...
struct PJRT_Memory_FunctionTable {
  ...
  void* (*get_user_data)(struct PJRT_Memory* memory, void* key);
  // Attaches user data to the PJRT_Memory implementation.
  void (*set_user_data)(struct PJRT_Memory* memory, void* key, void* data,
                        void (*dtor)(void*));
};
PJRT_DEFINE_STRUCT_TRAITS(PJRT_Memory_FunctionTable, set_user_data);

struct PJRT_Memory {
  struct PJRT_Memory_FunctionTable* vtable;
};
