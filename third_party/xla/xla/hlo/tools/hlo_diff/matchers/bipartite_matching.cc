...
void MatchInstructionsByShape(
  ...
  for (const auto& [shape, _] : left_instructions_by_shape) {
...
void MatchInstructions(
  ...
  std::vector<HloOpcode> sorted_opcodes;
  for (const auto& [opcode, _] : instructions_by_opcode) {
    sorted_opcodes.push_back(opcode);
  }
